...
export async function POST(request, { params }) {
  ...
  if (!sub.email) return withCors(NextResponse.json({ error: 'Subscriber has no email on file' }, { status: 400 }), request);
  if (sub.digital?.hasClaimed) return withCors(NextResponse.json({ error: 'Already claimed' }, { status: 400 }), request);
  ...
  try {
    ...
    return withCors(NextResponse.json({ error: `Failed to generate sign-in link: ${err.message}` }, { status: 500 }), request);
  }
  ...
  try {
    ...
    return withCors(NextResponse.json({ error: `Email send failed: ${err.message}` }, { status: 502 }), request);
  }
